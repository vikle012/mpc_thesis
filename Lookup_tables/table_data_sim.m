...
X = [x1_table_data(1,index); x2_table_data(1,index); x3_table_data(1,index); ...
     x4_table_data(1,index); x5_table_data(1,index)];
...
% 
% simU.n_e=[0 n_e]; 
% simU.u_delta=[0 u_ref(1)];
% simU.u_egr=[0 u_ref(2)];        
% simU.u_vgt=[0 u_ref(3)];
% 
% % --- Simulink initialization ---
% model.p_im_Init          = x(1);
% model.p_em_Init          = x(2);
% model.X_Oim_Init         = x(3); % Need to change 
% model.X_Oem_Init         = x(4); % init in model as well
% model.omega_t_Init       = x(5);
% model.utilde_egr1_Init   = 0;
% model.utilde_egr2_Init   = 0;
% model.utilde_vgt_Init    = 0;
% 
