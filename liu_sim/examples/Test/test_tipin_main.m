...
x0 = [cell2mat(engine_map.state_calibration(1, N_index)); 400];
u0 = cell2mat(engine_map.control_calibration(1, N_index));

% Terminate the optimzation at the highest load in the map
xT = [cell2mat(engine_map.state_calibration(end, N_index)); 800];
uT = cell2mat(engine_map.control_calibration(end, N_index));

% Formulate OCP
ocp = test_tipin_ocp(x0, u0, xT, uT, N_ice_opt, pwl_param);

% Simulate the system to get inital guess
v0 = test_tipin_initial_guess(x0, u0, N_ice_opt, pwl_param);
