function [dX, constraints, signals, param] = test_liu_diesel_2(X, U, N_ice, param)
...
T_em  = T_cyl_out(p_im, p_em, param.T_im, W_f, W_comp, param ); % [K]
...
W_turb = p_em./1e3./sqrt(T_em).*turbine_massflow(Pi_turb, 60.*Nt./sqrt(T_em), param); % [kg/s]
BSR    = turbine_BSR(Pi_turb, w_t, T_em, param); % [-]
eta_tm = turbine_efficiency(BSR, (Nt.*60)/sqrt(T_em), param); % [-]
% HÄR KAN MAN LÄGGA TILL BERÄKNING AV TEMPERATUR EFTER TURBIN, KANSKE KAN
% ANVÄNDA eta_tm ???
...
% BÖR ÄVEN WASTEDATEN TAS HÄNSYN TILL I BERÄKNING ???
...
signals=struct(...
    ...
    'W_turb'  ,W_turb , ...
    'BSR'     ,BSR    , ...
    'P_turb_eta_tm', P_turb_eta_tm, ... % HÄR BÖR MAN KUNNA ADDERA TEMP EFTER TURBIN
    'T_turb'    ,T_at   , ...
