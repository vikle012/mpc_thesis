...
%   .m files:
%       createParam__hev_tipin
%       ocp__hev_tipin
%   	initGuess__hev_tipin
%       yopSolve_tipin
%       PT_PWL__hev_tipin
%       internalSignals__hev_tipin
%       whtcPlots
%       ld2Plots
%       noxPlots
%       emPlots
...
tf      = 466;%1114-648;%248;            % End time   [s]
...
if ~exist('param', 'var')
    regen_brake = 1000; % Nm
    param = SPECIAL_createParam__hev_eats_tipin(engine_map, regen_brake);
end
...
% Creating OCP
[ocp, info.L, info.E] = SPECIAL_ocp__hev_eats_tipin(x0, u0, xT, uT, t0, tf, param);
% Initial guess
v0 = SPECIAL_initGuess__hev_eats_tipin(x0, u0, t0, tf, param);
% test_check_v0;
% Creating NLP and solving it
% Optimal solution is found when objective is within 'tol'.
nlpOpts.ipopt.constr_viol_tol   = 1e-5;
...
hev = @(x, u, t) PT_PWL__hev_eats_tipin(x, u, t, param, t0);
opt_signals = internalSignals__hev_eats_tipin(hev, t_opt, x_opt, u_opt);
