...
%           powertrain is optimally controlled for fastest possible lightoff
%           in a fixed operating point. Throttle is assumed fully open to
...
if ~exist('param', 'var')
    regen_brake = 1000; % Nm
    param = createParam__hev_eats_lightoff(engine_map, regen_brake);
end
...
xT = [param.ld2.constr.x_min(1), param.ld2.constr.x_max(1); ... p_cac [0.1, 5] bar
    ...
    473, param.ld2.constr.x_max(15); ...param.ld2.constr.x_max(15); ...
...
% Creating OCP
[ocp, info.L, info.E] = ocp__hev_eats_lightoff(x0, u0, xT, uT, t0, tf, param);
% Initial guess
v0 = initGuess__hev_eats_lightoff(x0, u0, t0, tf, param);
...
fprintf('Total NOx: %.4f g\n', trapz(opt_signals.t_vec.val,opt_signals.NOx_TP_Cu.val));
