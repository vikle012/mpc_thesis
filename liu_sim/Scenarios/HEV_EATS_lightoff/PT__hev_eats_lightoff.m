function [dX, constraints, signals, param] = PT__hev_eats_lightoff(X, U, t, param, t0)
% FUNCTION: Wrapper for the model equations describing the dynamics of the
%           model.
%
% SYNTAX: [output1, output2, output3, output4] =
%           initGuess__hev_lightoff(input1, input2, input3, input4, input5);
%
% INPUTS:
...
%
% EXAMPLE:
%   hev = @(x,u,t) PT__hev_lightoff(x, u, t, param, 0);
...
M_dem = 250;%800;%full(lut_M(t+t0));

constraints = [c_ld2; ...
    c_em; ...
    (M_dem .* 0.99) - signals.ld2.M_ice - U(M_em); ... Lower limit
