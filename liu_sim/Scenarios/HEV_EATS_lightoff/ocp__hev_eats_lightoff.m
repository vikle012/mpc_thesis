function [ocp, L, E] = ocp__hev_eats_lightoff(x0, u0, xT, uT, t0, tf, param)
% FUNCTION: Creates an optimal control problem 'ocp' to be used in the
%           optimization.
%
% SYNTAX: [output1] = ocp__hev_lightoff(input1, input2, input3, input4, ...
...
%
% EXAMPLE:
%   ocp = ocp__hev_lightoff(x0, u0, xT, uT, 0, 1800, param)
%
...
% April 2018; Last revision: 20-April-2018
...
[xdot, h, signals] = PT_PWL__hev_eats_lightoff(x, u, t, param, t0);
...
E = 'tf=850';
beta = 0.35;
phaseData={'L', signals.NOx_TP_Cu ...1*(1-beta) + beta*signals.NOx_TP_Cu*40 ... + signals.W_f  ... Integral cost (1 = time)
    ...'E', signals.NOx_tot ...
    'tf_min', tf ... End time lower bound
    'tf_max', tf+100 ... End time upper bound
    'f', xdot     ... Dynamics
...
ocpDesc={'state_norm', param.ld2.ocp.state_norm, ... % State normlization
    ...
    'obj_norm', 50}; % Obj. fn normalization
