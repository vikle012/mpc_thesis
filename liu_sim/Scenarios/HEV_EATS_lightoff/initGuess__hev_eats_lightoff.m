function v0 = initGuess__hev_eats_lightoff(x0, u0, t0, tf, param)
% FUNCTION: Simulating guessed control signals for initial guess of states
%           for the OCP.
%
% SYNTAX: output1 = initGuess__hev_eats_whtc(input1, input2, input3,
%                                          	 input4, input5);
%
% INPUTS:
...
%
% EXAMPLE:
%   v0 = initGuess__hev_eats_whtc(x0, u0, 45, 1170, param);
%
% OTHER FILES REQUIRED:
%   .m files:
%       PT_IG__hev_eats_whtc
%       yopInitialGuess
%
%   .mat files:
%       none
...
%     7.42584402079555], x0(6,1));
% 
% Initial guess of fuel injection based on torque request
% u_f_init = max(0, M_dem ./ 10 + ss_fuel);            % TEST
...
hev = @(t,x) PT_IG__hev_eats_lightoff(x, u, t, param);
...
% end
% u_sim(1,M_em)= -300;
% u_sim(1,u_f) = max(0, (300+250) ./ 10 + 8);
% u_guess(1,M_em) = 1;
% u_guess(1,u_f)  = -0.1;
% for i = 2:length(t_sim)
%     %u_f och du_f
%     u_sim(i,M_em) = 1*t_sim(i) - 300;
%     u_sim(i,u_f) = max(0, (250 - u_sim(i,M_em)) ./ 10 + 8);
