...
if ~exist('param', 'var')
    param = createParam__ld2_nox_EATS_SS(engine_map);
end
...
% % States (( NEEDS A SPAN TO FUNCTION ))
...
%     ];
% % Controls
% u0 = [engine_map.control_calibration{M_index_init, N_index_init}(1).*0.8, ... u_f min
...
xT = [param.ld2.constr.x_min(1), param.ld2.constr.x_max(1); ... p_cac [0.1, 5] bar
    ...
    param.ld2.constr.x_min(7:13), param.ld2.constr.x_max(7:13); ... Temperatures
...
% Creating OCP
[ocp, info.L, info.E] = SPECIAL_ocp__ld2_nox_EATS_SS(x0, u0, xT, uT, t0, tf, param);

% Initial guess
v0 = initGuess__ld2_nox_EATS_SS(x0, u0, t0, tf, param);
...
ld2_NOx = @(x, u, t) PT_PWL__ld2_nox_EATS_SS(x, u, t, param, t0);
opt_signals = internalSignals__ld2_nox_EATS_SS(ld2_NOx, t_opt, x_opt, u_opt);
