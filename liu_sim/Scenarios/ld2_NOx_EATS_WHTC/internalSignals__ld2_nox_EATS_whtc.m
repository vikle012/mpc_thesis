function z = internalSignals__ld2_nox_EATS_whtc(ld2_NOx_EATS, t_vec, x_vec, u_vec, t_label)

z = struct;

[~, ~, ~, parameters_used] = ld2_NOx_EATS(x_vec(1,:)', u_vec(1,:)', t_vec(1));
...
for k = 1:length(t_vec)
    ...
    [~, ~, c] = ld2_NOx_EATS(x_vec(k,:)', u_vec(k,:)', t_vec(k));
    ...
    z.T_atw_c.val(k)      = c.T_atw - 273;
    z.T_tw_drop.val(k)  = c.T_tw_drop;
    z.W_tw.val(k)       = c.W_tw;
    % Engine out emissions
    z.C6 = 'Engine out emissions';
...
z.NOx_tot.title = 'Tailpipe NO_{x}';
...
z.deNOx.title  = 'NOx conversion';
z.deNOx.label  = 'NOx conversion [-]';
z.NOx_TP.title  = 'Tailpipe NOx';
z.NOx_TP.label  = 'NOx [kg]';
z.T_SCR1.title    = 'SCR1';
