...
if ~exist('param', 'var')
    param = createParam__ld2_nox_whtc(engine_map);
end
...
uT = [param.ld2.constr.x_min(7), param.ld2.constr.x_max(7); ... w_f [0, 280]
    param.ld2.constr.x_min(8), param.ld2.constr.x_max(8); ...   u_th [0, 1]
    param.ld2.constr.x_min(9), param.ld2.constr.x_max(9); ...   u_wg [0, 1]
    ];

%% SOLVE FOR OPTIMAL SOLUTION

% Creating OCP
[ocp, info.L, info.E] = ocp__ld2_nox_whtc(x0, u0, xT, uT, t0, tf, param);

% Initial guess
v0 = initGuess__ld2_nox_whtc(x0, u0, t0, tf, param);
...
ld2_NOx = @(x, u, t) PT_PWL__ld2_nox_whtc(x, u, t, param, t0);
opt_signals = internalSignals__ld2_nox_whtc(ld2_NOx, t_opt, x_opt, u_opt);
