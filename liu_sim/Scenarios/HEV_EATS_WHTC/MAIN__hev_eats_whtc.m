...
K       = 0.5*(tf-t0);    % Collocation segments (normally 1 or 2 per second)
...
xT = [param.ld2.constr.x_min(1), param.ld2.constr.x_max(1); ... p_cac [0.1, 5] bar
    ...
    0.375, 0.375; ...                                               SOC [0.25 0.5]
    param.ld2.constr.x_min(8:19), param.ld2.constr.x_max(8:19) ...
    ];
...
% Creating OCP
[ocp, info.L, info.E] = ocp__hev_eats_whtc(x0, u0, xT, uT, t0, tf, param);
% Initial guess
v0 = initGuess__hev_eats_whtc(x0, u0, t0, tf, param);
...
hev = @(x, u, t) PT_PWL__hev_eats_whtc(x, u, t, param, t0);
opt_signals = internalSignals__hev_eats_whtc(hev, t_opt, x_opt, u_opt);
