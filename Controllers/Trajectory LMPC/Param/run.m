...
ref.x_ref = [x_800 x_800 x_900 x_900]; % x_ref;
ref.u_ref = [u_800 u_800 u_900 u_900]; % u_ref;
ref.time = [1:4]; % time;
...
simulate.x_ref = timeseries([x_800'; x_800'; x_900'; x_900'; x_900'], time_vector);
simulate.u_ref = timeseries([u_800'; u_800'; u_900'; u_900'; u_900'], time_vector);

% Initialization
% Note: change in casadi_MPC.m aswell
simulate.p_im_Init          = ref.x_ref(1);
simulate.p_em_Init          = ref.x_ref(2);
simulate.X_Oim_Init         = ref.x_ref(3);
simulate.X_Oem_Init         = ref.x_ref(4);
simulate.omega_t_Init       = ref.x_ref(5);
simulate.utilde_egr1_Init   = 0;
simulate.utilde_egr2_Init   = 0;
simulate.utilde_vgt_Init    = 0;

[~,y,~] = diesel_engine(ref.x_ref(:,1), ref.u_ref(:,1), tuning_param.n_e(1), model);
simulate.x_r_Init = y.x_r;
simulate.T_1_Init = y.T_1;
simulate.u_egr_Init = ref.u_ref(2);
simulate.u_vgt_Init = ref.u_ref(3);

% Simulation
sim('traj_model', simulation_time)
...
figure(1)
subplot(511)
plot(tout, linspace(x_800(1), x_800(1), length(tout)), 'k--')
hold on
plot(tout, linspace(x_900(1), x_900(1), length(tout)), 'k:')
...
subplot(512)
plot(tout, linspace(x_800(2), x_800(2), length(tout)), 'k--')
hold on
plot(tout, linspace(x_900(2), x_900(2), length(tout)), 'k:')
plot(tout, simp_em)
...
subplot(513)
plot(tout, linspace(x_800(3), x_800(3), length(tout)), 'k--')
hold on
plot(tout, linspace(x_900(3), x_900(3), length(tout)), 'k:')
plot(tout, simX_Oim)
...
subplot(514)
plot(tout, linspace(x_800(4), x_800(4), length(tout)), 'k--')
hold on
plot(tout, linspace(x_900(4), x_900(4), length(tout)), 'k:')
plot(tout, simX_Oem)
...
subplot(515)
plot(tout, linspace(x_800(5), x_800(5), length(tout)), 'k--')
hold on
plot(tout, linspace(x_900(5), x_900(5), length(tout)), 'k:')
...
figure(2)
subplot(311)
plot(tout, linspace(u_800(1), u_800(1), length(tout)), 'k--')
hold on
plot(tout, linspace(u_900(1), u_900(1), length(tout)), 'k:')
...
subplot(312)
plot(tout, linspace(u_800(2), u_800(2), length(tout)), 'k--')
hold on
plot(tout, linspace(u_900(2), u_900(2), length(tout)), 'k:')
...
subplot(313)
plot(tout, linspace(u_800(3), u_800(3), length(tout)), 'k--')
hold on
plot(tout, linspace(u_900(3), u_900(3), length(tout)), 'k:')
...
figure(3)
plot(tout, linspace(800, 800, length(tout)), 'k--')
hold on
plot(tout, linspace(900, 900, length(tout)), 'k:')
plot(tout, M_e)
title('M_e')